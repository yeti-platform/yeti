...
class ThreatInsight(Feed):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config = {
            ...
            "import_email_metadata": yeti_config.get(
                "proofpoint", "import_email_metadata"
            ),
        }

    def update(self):
        self.api = ProofPointAPI(
        ...
        log.info(
            "Downloaded {nb} message threats for the last period".format(
        ...
        for threat in campaign_info["campaignMembers"]:
            ...
            try:
                ...
                try:
                    ...
                    # wtf
                    log.error(
                        "{cls} {v} has a weird problem - FIXME".format(cls=cls, v=v)
                    )
    ...
    def _make_threat_nodes(threat, context, tags):
        ...
        if threat["threatType"] == "url":
            if threat["classification"] == "phish":
                ...
                log.info(
                    "URL threat - ignore classification %s", threat["classification"]
                )
            else:
                log.error(
                    "Type: url, Unsupported classification %s", threat["classification"]
                )
                log.debug(pprint.pformat(threat))
                return None
            threats["url"] = threat
    ...
    def _make_entities(self, threat):
        ...
        for ttp in campaign_info["techniques"]:
            _t = TTP.get_or_create(
                name=ttp["name"], killchain="3", tags=[self.name]
            ).save()
            # _t.killchain = "3"
    ...
    def _get_threat_forensics_nodes(self, threat, threat_nodes, general_context, tags):
        ...
        results = self.forensic_api.get_reports_from_threat(
        ...
        for _id, evidence_list in evidences.items():
            ...
            for evidence in evidence_list:
                try:
                    nodes = self._get_threat_forensics_nodes_inner(
                        evidence, general_context, tags
                    )
    ...
    def _get_threat_forensics_nodes_inner(self, evidence, general_context, tags):
        ...
        tags = [
            {"name": _} for _ in set([evidence["type"]] + [d["name"] for d in tags])
        ]
        # create Tags in DB
        for _ in tags:
        ...
        if evidence["type"] in ["file", "dropper"]:
            ...
            if "md5" in evidence["what"]:
                threat_forensics.append(
                    Hash.get_or_create(value=evidence["what"]["md5"], context=[context])
                )
            if "sha256" in evidence["what"]:
                threat_forensics.append(
                    Hash.get_or_create(
            ...
            threat_forensics.append(
                Text.get_or_create(value=evidence["what"]["ids"], context=[context])
            )
        elif evidence["type"] == "mutex":
            threat_forensics.append(
                Text.get_or_create(value=evidence["what"]["name"], context=[context])
            )
        elif evidence["type"] == "network":
            if "ip" in evidence["what"]:
                # FIXME port, type
                threat_forensics.append(
                    Ip.get_or_create(value=evidence["what"]["ip"], context=[context])
                )
            elif "domain" in evidence["what"]:
                threat_forensics.append(
                    Hostname.get_or_create(
            ...
            threat_forensics.append(
                Url.get_or_create(value=evidence["what"]["url"], context=[context])
            )
            ...
            if self.config["import_email_metadata"]:
                ...
                if "headerFrom" in msg:
                    ...
                    _s2 = Email.get_or_create(
                        value=msg["headerFrom"], created=create_t, context=[context]
                    )
                    _s2.tag(["sender"])
...
class ForensicsAPI:
    ...
    def get_reports_from_threat(self, threat_id, with_campaign_forensics=False):
        return _json(
            self.__api.forensics.GET(
                params={
                    "threatId": threat_id,
                    "includeCampaignForensics": str(with_campaign_forensics).lower(),
                }
            )
        )
...
class SIEMAPI:
    ...
    @staticmethod
    def make_time_param(
        frequency=None, interval=None, since_seconds=None, since_time=None
    ):
