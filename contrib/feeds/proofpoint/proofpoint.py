...
class ThreatInsight(Feed):
    ...
    default_values = {
        ...
        "description":
            "This feed contains sandbox observables from blocked emails by Proofpoint",
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config = {
            'api_user':
                yeti_config.get('proofpoint', 'api_user'),
            'api_password':
                yeti_config.get('proofpoint', 'api_password'),
            'tenant_id':
                yeti_config.get('proofpoint', 'tenant_id'),
            'import_email_metadata':
    ...
    def update(self):
        ...
        for threat in threats:
            ...
            events = self._get_messages_for_threat(
                messages['messagesBlocked'], threat)
...
#                    value=threats['url']['threat'], context=[context]))
    ...
    def _get_threat_forensics_nodes(
            self, threat, threat_nodes, general_context, tags):
        ...
        for _id, evidence_list in evidences.items():
            ...
            for evidence in evidence_list:
                try:
                    ...
                    log.error(
                        'threat_forensics threatID was %s', threat['threatID'])
...
class ForensicsAPI:
            ...
            self.__api.forensics.GET(
                params={
                    ...
                    'includeCampaignForensics':
                        str(with_campaign_forensics).lower()
                }))
    ...
    def get_reports_from_campaign(self, campaign_id):
        return _json(
            self.__api.forensics.GET(params={
