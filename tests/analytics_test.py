...
class AnalyticsTest(unittest.TestCase):
    ...
    def test_censys_query(self, mock_censys_hosts):
        ...
        censys_query = indicator.Query(
            ...
            query_type=indicator.QueryType.censys).save()


        mock_search_result = [{'ip': '192.0.2.1'}, {'ip': '2001:db8:3333:4444:5555:6666:7777:8888'}]
        ...
        mock_hosts_api.search.assert_called_once_with('test_censys_query', fields=['ip'], pages=-1)

        observables = observable.Observable.filter({"value": ''}, graph_queries=[('tags', 'tagged', 'outbound', 'name')])
        ...
        self.assertEqual(observable_obj[1].value, '2001:db8:3333:4444:5555:6666:7777:8888')
