...
class HybridAnalysis(task.FeedTask):
    ...
    def analyze(self, item):
        ...
        try:
            ...
            logging.exception(f"HybridAnalysis: Failed to save md5: {item['md5']}")
        ...
        try:
            ...
            logging.exception(f"HybridAnalysis: Failed to save sha1: {item['sha1']}")
        ...
        if "domains" in item and isinstance(item["domains"], list):
            for domain in item["domains"]:
                try:
                    ...
                    logging.exception(
                        f"HybridAnalysis: Failed to save domain: {domain}"
                    )
        ...
        if "extracted_files" in item and isinstance(item["extracted_files"], list):
            for extracted_file in item["extracted_files"]:
                ...
                try:
                    ...
                    logging.exception(
                        f"HybridAnalysis: Failed to save file: {extracted_file['sha256']}"
                    )
                    continue
                ...
                if (
                    ...
                    try:
                        ...
                        logging.exception(
                            f"HybridAnalysis: Failed to save path: {extracted_file['file_path']}"
                        )
