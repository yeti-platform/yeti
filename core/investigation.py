...
class Investigation(Node):
    ...
    def get_form(klass):
        ...
        form.created_by = WTFHiddenField("created_by", default=current_user.username)
    ...
    def sharing_permissions(self, sharing_with, investigation=False, invest_id=False):
        ...
        if sharing_with == "all":
            ...
            Investigation.objects.get(id=invest_id or self.id).update(
                add_to_set__sharing=[current_user.id]
            )
        ...
        if groups:
            Investigation.objects.get(id=self.id).update(
                add_to_set__sharing=[group.id for group in groups]
            )
